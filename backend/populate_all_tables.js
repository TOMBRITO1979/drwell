...
async function main() {
    ...
    const hashedSuperAdminPassword = await bcryptjs_1.default.hash(superAdminPassword, 10);
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const user = await prisma.user.create({
                data: {
                    ...
                    password: await bcryptjs_1.default.hash(userPassword, 10),
                    name: userNames[i],
    ...
    const allCases = [];
    const casesBase = [
        { processNumber: '1000123-45.2024.8.26.0100', court: 'TJSP - 1ª Vara Cível', subject: 'Ação de Cobrança', value: 50000.00, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '2000234-56.2024.8.26.0100', court: 'TRT 2ª Região - 5ª Vara do Trabalho', subject: 'Reclamação Trabalhista', value: 75000.00, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '3000345-67.2023.8.26.0100', court: 'TJSP - 2ª Vara de Família', subject: 'Divórcio Consensual', value: 0, status: client_1.CaseStatus.FINISHED },
        { processNumber: '4000456-78.2024.8.26.0100', court: 'TJSP - 10ª Vara Criminal', subject: 'Defesa Criminal', value: 0, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '5000567-89.2024.8.26.0100', court: 'TJSP - 3ª Vara Cível', subject: 'Indenização por Danos Morais', value: 30000.00, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '6000678-90.2023.8.26.0100', court: 'TJSP - 1ª Vara Empresarial', subject: 'Dissolução de Sociedade', value: 500000.00, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '7000789-01.2024.8.26.0100', court: 'TJSP - 4ª Vara Cível', subject: 'Cobrança de Aluguel', value: 15000.00, status: client_1.CaseStatus.ARCHIVED },
        { processNumber: '8000890-12.2024.8.26.0100', court: 'TJSP - Juizado Especial Cível', subject: 'Defeito em Produto', value: 8000.00, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '9000901-23.2024.8.26.0100', court: 'TJSP - 5ª Vara Cível', subject: 'Ação Possessória', value: 120000.00, status: client_1.CaseStatus.ACTIVE },
        { processNumber: '1001012-34.2024.8.26.0100', court: 'TJSP - 6ª Vara Cível', subject: 'Revisão de Contrato', value: 45000.00, status: client_1.CaseStatus.ACTIVE },
    ];
    let caseIndex = 0;
    ...
    for (const caseRecord of allCases) {
        ...
        await prisma.casePart.create({
            data: {
                ...
                type: client_1.CasePartType.AUTOR,
        ...
        await prisma.casePart.create({
            data: {
                caseId: caseRecord.id,
                type: client_1.CasePartType.REU,
        ...
        if (Math.random() > 0.5) {
            await prisma.casePart.create({
                data: {
                    caseId: caseRecord.id,
                    type: client_1.CasePartType.REPRESENTANTE_LEGAL,
...
════════════════════════════════════════════════════════════════
`;
    (0, fs_1.writeFileSync)('/tmp/CREDENTIALS_COMPLETE.txt', credentialsOutput);
